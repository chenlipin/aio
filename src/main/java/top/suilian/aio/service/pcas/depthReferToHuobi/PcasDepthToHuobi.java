package top.suilian.aio.service.pcas.depthReferToHuobi;
...
public class PcasDepthToHuobi extends PcasParentService {

    public PcasDepthToHuobi(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_PCAS_REFER_DEPTH, id);
    }
    ...
    public void init() throws UnsupportedEncodingException {
        ...
        String trades = httpUtil.get(referBaseUrl+"/market/depth?symbol="+symbol1+"&&depth=20&type=step0");
        JSONObject tradesObj = judgeRes(trades, "tick", "getRandomPrice");

        if (!"".equals(trades) && trades != null && !trades.isEmpty() && tradesObj != null) {
            JSONObject depthJson=tradesObj.getJSONObject("tick");

            List<List<String>> buyPrices = (List<List<String>>) depthJson.get("bids");
            logger.info("对标买单:" + buyPrices);


            List<List<String>> sellPrices = (List<List<String>>) depthJson.get("asks");
            ...
            if (jsonOrders != null && jsonOrders.getString("code").equals("0000")) {
                ...
                if(resultList.size()==0){
                    logger.info("当前挂单数:" + resultList.size());
                }else{
            ...
            for (int i = 0; i < sellNum; i++) {
                sleep(700, Integer.parseInt(exchange.get("isMobileSwitch")));
...
//                setTradeLog(id, "对标挂买单", 0, "05cbc8");
        ...
        clearLog();
        sleep(5000, Integer.parseInt(exchange.get("isMobileSwitch")));
