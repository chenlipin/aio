...
import top.suilian.aio.service.fchain.kline.FChainKline;
...
@Component
public class RunNewFchainKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        NewFchainKline kline = new NewFchainKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        NewFchainKline kline;

        public Work(NewFchainKline kline) {
            super(kline.id);
            this.kline = kline;
        }
