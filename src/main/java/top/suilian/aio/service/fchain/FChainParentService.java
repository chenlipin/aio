...
public class FChainParentService extends BaseService {
    public String baseUrl = "https://api.fchain.one";
    ...
    public void setBalanceRedis() throws UnsupportedEncodingException {
        ...
        if (balance == null || overdue) {
            ...
            String res = httpUtil.doGet("https://api.fchain.one/openapi/v1/account?" + par, exchange.get("apikey"));
        ...
        if (!rt.equals("") && rtObj != null) {
            ...
            for (Map<String, Object> data : symbols) {
                if (exchange.get("market").toUpperCase().equals(data.get("symbol"))) {
                    ...
                    String minPrice = filtersArr.getJSONObject(0).getString("minPrice");
                    int minPriceIndex = minPrice.indexOf(".");


                    String minQty = filtersArr.getJSONObject(1).getString("minQty");
                    int minQtyIndex = minQty.indexOf(".");

                    precision.put("pricePrecision", minPrice.length() - minPriceIndex - 1);

                    if (exchange.get("market").toUpperCase().equals(data.get("SCCDRINK"))) {
                        precision.put("amountPrecision", 1);
                    } else {
                        precision.put("amountPrecision", minQty.length() - minQtyIndex - 1);
                    }
