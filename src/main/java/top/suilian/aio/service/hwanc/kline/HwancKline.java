package top.suilian.aio.service.hwanc.kline;
...
import top.suilian.aio.service.hwanc.HwancParentService;
...
public class HwancKline extends HwancParentService {
    public HwancKline(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_HWANC_KLINE, id);
    }
    ...
    public void init() {
        ...
        if (runTime < Integer.parseInt(exchange.get("timeSlot"))) {
            ...
            JSONObject tradesObj = judgeRes(trades, "buy", "getRandomPrice");

            if (tradesObj != null && tradesObj.getInt("code") == 200) {

                JSONObject datas = tradesObj.getJSONObject("datas");

                List<Map<String, String>> buyPrices = (List<Map<String, String>>) datas.get("buy");

                List<Map<String, String>> sellPrices = (List<Map<String, String>>) datas.get("sell");

                BigDecimal buyPri = new BigDecimal(String.valueOf(buyPrices.get(0).get("p")));
                BigDecimal sellPri = new BigDecimal(String.valueOf(sellPrices.get(0).get("p")));
            ...
            try {
                ...
                if (jsonObject != null && jsonObject.getInt("code") == 200) {
                    ...
                    if (jsonObject.getInt("code") == 200) {
                        ...
                        String tradeId = datas.getString("ID");
                        orderIdOne = tradeId;
                        ...
                        if (jsonObject1 != null && jsonObject1.getInt("code") == 200) {
                            JSONObject datas2 = jsonObject1.getJSONObject("datas");
                            orderIdTwo = datas2.getString("ID");
    ...
    public BigDecimal getRandomPrice() {
        ...
        if (tradesObj != null && tradesObj.getInt("code") == 200) {
            ...
            //吃堵盘口的订单
            BigDecimal buyAmount = new BigDecimal(String.valueOf(buyPrices.get(0).get("a")));
            BigDecimal sellAmount = new BigDecimal(String.valueOf(sellPrices.get(0).get("a")));
            int maxEatOrder = Integer.parseInt(exchange.get("maxEatOrder"));//吃单成交上限数
            if(maxEatOrder==0){
    ...
    public void openInterval(BigDecimal sellPrice, List<Map<String, String>> allBids, BigDecimal openIntervalPrice) {

        BigDecimal price;
        for (Map<String, String> bid : allBids) {

            price = new BigDecimal(String.valueOf(bid.get("p")));
            if (price.compareTo(sellPrice.subtract(openIntervalPrice)) < 0) {
                break;
            }
            if (new BigDecimal(exchange.get("openIntervalAllAmount")).compareTo(intervalAmount.add(new BigDecimal(String.valueOf(bid.get("a"))))) < 0) {
                setRobotArgs(id, "isOpenIntervalSwitch", "0");
                setTradeLog(id, "刷开区间的数量已达到最大值,停止刷开区间", 0, "000000");
                break;
            }
            //开始挂单
            startOpenInterval(price, new BigDecimal(String.valueOf(bid.get("a"))));
        }
    }
