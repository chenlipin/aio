package top.suilian.aio.service.bihu.kline;
...
public class BiHuKline extends BiHuParentService {
    public BiHuKline(
            ...
            try {
                ...
                if (jsonObject != null && jsonObject.getInt("error") == 0) {
                    ...
                    if (jsonObject1 != null && jsonObject1.getInt("error") == 0) {
                        ...
                        String res = cancelTrade(tradeId);
                        JSONObject cancelRes = JSONObject.fromObject(res);
                        setCancelOrder(cancelRes, res, tradeId, Constant.KEY_CANCEL_ORDER_TYPE_QUANTIFICATION);
                        setTradeLog(id, "撤单[" + tradeId + "]=> " + res, 0, "000000");
                    }
                } else if (jsonObject != null && jsonObject.getInt("error") == 1) {
            ...
            if (buyAmount.compareTo(new BigDecimal(exchange.get("buyMinLimitAmount"))) < 1 && maxEatOrder == 0) {
                ...
                try {
                   ...
                   // JSONObject jsonObject = judgeRes(sellOrder, "error", "submitTrade");
                    JSONObject jsonObject = JSONObject.fromObject(sellOrder);
                    if (jsonObject != null && jsonObject.getInt("error") == 0) {
                        String tradeId = jsonObject.getString("data");
                        sellOrderId = tradeId;
                    }
            ...
            if (sellAmount.compareTo(new BigDecimal(exchange.get("sellMinLimitAmount"))) < 1 && maxEatOrder == 0) {
                ...
                try {
                  ...
                  //  JSONObject jsonObject = judgeRes(buyOrder, "error", "submitTrade");
                    JSONObject jsonObject = JSONObject.fromObject(buyOrder);
                    if (jsonObject != null && jsonObject.getInt("error") == 0) {
                        String tradeId = jsonObject.getString("data");
                        buyOrderId = tradeId;
                    }
