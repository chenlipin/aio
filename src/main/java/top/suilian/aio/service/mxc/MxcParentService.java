...
public class MxcParentService  extends BaseService{

        public String baseUrl = "https://www.mxc.io";
    ...
    public String getDepth(){
        ...
        String trade = httpUtil.get(baseUrl+"/open/api/v1/data/depth?depth=10&market="+exchange.get("market"));
    ...
    public   boolean setPrecision() {
        ...
        String rt = httpUtil.get(baseUrl+"/open/api/v1/data/markets_info");

        JSONObject rtObj = JSONObject.fromObject(rt);
        logger.info("获取交易规则："+rtObj);
        if (!rt.equals("") && rtObj != null) {
            JSONObject dataJson = rtObj.getJSONObject("data");
            JSONObject precisionJson =dataJson.getJSONObject(exchange.get("market"));
            if(!"".equals(precisionJson)&&precisionJson!=null){
                String amountPrecision = precisionJson.getString("quantityScale");
                String pricePrecision = precisionJson.getString("priceScale");
