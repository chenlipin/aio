...
import top.suilian.aio.service.golden.GoldenParentService;

import java.io.UnsupportedEncodingException;
import java.util.List;

public class GoldenCancel extends GoldenParentService {
    public GoldenCancel(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_GOLDEN_CANCEL, id);
    }
    ...
    public void init() {
        ...
        List<CancelOrder> cancelOrderList = cancelOrderService.findByAll(Constant.KEY_EXCHANGE_GOLDEN, robotId);
        ...
        for (CancelOrder cancelOrder : cancelOrderList) {
            ...
            try {
                ...
                if (orderDetialRes.getString("status").equals("success")){
                    JSONObject data = orderDetialRes.getJSONObject("data");
                    if (" WITHDRAWN".equals(data.get("status"))) {  //订单撤销
                        status = Constant.KEY_CANCEL_ORDER_STATUS_CANCELLED;
                    } else if ("COMPLE-TRADE".equals(data.get("status"))) {  //订单完全成交
...
//                    }
                        ...
                        if (cancelDetailRes.getString("status").equals("status")) {
                            status = Constant.KEY_CANCEL_ORDER_STATUS_CANCELLED_BY_THREAD;
                            remark = cancelDetail;
                        } else {
                ...
                else if (orderDetialRes.getString("status").equals("entrust_failure_0016")) {       //订单不存在
