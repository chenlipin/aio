...
@Component
public class RunNewBitMartKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        NewBitMartKline kline = new NewBitMartKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        NewBitMartKline kline;

        public Work(NewBitMartKline kline) {
            super(kline.id);
            this.kline = kline;
        }
