package top.suilian.aio.service.bitmart.depthReferToZg;
...
public class DepthHotcoinReferToZg extends BitMartParentService {
    public DepthHotcoinReferToZg(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_HOTCOIN_REFER_DEPTH, id);
    }
    ...
    public void init() throws UnsupportedEncodingException {
        ...
        //获取zg深度
        String trades = httpUtil.get(referBaseUrl + "/depth?symbol=" + exchange.get("referSymbol") + "&size=" + exchange.get("depthNum"));
        ...
        if (!"".equals(trades) && trades != null && !trades.isEmpty() && tradesObj != null) {
            ...
            //获取自己的挂单
            String tradeOrders = getTradeOrders(1);
            ...
            if (jsonOrders != null && jsonOrders.getInt("code")==200) {

                JSONObject dataJson = jsonOrders.getJSONObject("data");
                JSONArray resultList = dataJson.getJSONArray("entrutsCur");
...
//                setTradeLog(id, "对标挂卖单amount:" + finalSellAmount + "--price:" + finalSellPrice, 0, "ff6224");
            ...
            for (int i = 0; i < ownSellOrders.size(); i++) {
                ...
                if (!has) {
                    String result=cancelTrade(tradeId);
                    setTradeLog(id, "撤单id["+tradeId+"],"+result, 0, "000000");
                }
