package top.suilian.aio.service.bitbaby.RandomDepth;
...
public class BitbabyRandomDepth extends BitbabyParentService {

    public BitbabyRandomDepth(
        ...
        super.id = id;
        super.logger = getLogger("bitbaby/depth", id);
    }
    ...
    public void init() {
        ...
        if (Integer.parseInt(exchange.get("depthCancelNum")) >= depthCancelOrderNum) {
            //获取深度
            List<BigDecimal> depthPrice = getDepthlist();
            logger.info("深度 获取的随机的买卖单价格:" + depthPrice);
            //买单数量
            BigDecimal buyNum = getOrderAmount();
            logger.info("深度买单价格:" + depthPrice.get(0)+"深度买单数量:" + buyNum);

            //卖单数量
            BigDecimal sellNum = getOrderAmount();
            logger.info("深度卖单价格:" + depthPrice.get(1)+"深度卖单数量:" + sellNum);
            ...
            try {
                ...
                if(resultBuy!=null){
                    setTradeLog(id, "深度 买单价格："+depthPrice.get(0)+",数量："+buyNum+",挂单结果："+buyResultObject.getJSONArray("orderId").getString(0), 0, "05cbc8");
                    JudegOrder(buyResultObject);
                }else{
                    setTradeLog(id, "深度 买单价格："+depthPrice.get(0)+",数量："+buyNum+",挂单结果："+resultBuy, 0, "05cbc8");
            ...
            try {
                ...
                JSONObject sellResultObject = judgeRes(resultSell, "status", "submitTrade");
                if(resultSell!=null){
                    setTradeLog(id, "深度 卖单价格："+depthPrice.get(1)+",数量："+sellNum+"，挂单结果："+sellResultObject.getJSONArray("orderId").getString(0), 0, "ff6224");
                    JudegOrder(sellResultObject);
                }else {
                    setTradeLog(id, "深度 卖单价格："+depthPrice.get(1)+",数量："+sellNum+"，挂单结果："+sellResultObject.getString("msg"), 0, "ff6224");
                    sleep( 10000, Integer.parseInt(exchange.get("isMobileSwitch")));
                }
            } catch (UnsupportedEncodingException e) {
                exceptionMessage = collectExceptionStackMsg(e);
    ...
    public void JudegOrder(JSONObject object) throws UnsupportedEncodingException {
        if (object != null ) {
            ...
            String tradeId = object.getJSONArray("orderId").getString(0);
            ...
            if (jsonObject != null ) {
                ...
                if (status.contains("FILLED")) {
                    ...
                    setTradeLog(id, "深度订单撤单[" + tradeId + "]=>" + cancelRes, 0, "000000");
                }
            }
        }
    }
    ...
    public List<BigDecimal> getDepthlist() {

        List<BigDecimal> price = new ArrayList<BigDecimal>();

        String depth = HttpUtil.get(baseUrl +"/spot/open/sapi/v1/depth?symbol="+exchange.get("market")+"&limit=10");

        //获取深度 判断平台撮合是否成功
        JSONObject tradesObj = judgeRes(depth, "bids", "getRandomPrice");
