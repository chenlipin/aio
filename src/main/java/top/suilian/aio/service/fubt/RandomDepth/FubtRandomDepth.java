package top.suilian.aio.service.fubt.RandomDepth;
...
import top.suilian.aio.service.fubt.FubtParentService;
...
public class FubtRandomDepth extends FubtParentService {
    public FubtRandomDepth(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_FUBT_DEPTH, id);
    }
    ...
    public void init() {
        ...
        if (Integer.parseInt(exchange.get("depthCancelNum")) >= depthCancelOrderNum) {
            ...
            try {
                ...
                if (resultBuy != null && buyResultObject.getString("status").equals("success")) {
            ...
            try {
                ...
                if (resultSell != null && sellResultObject.getString("status").equals("success")) {
                    setTradeLog(id, "卖单价格："+depthPrice.get(0)+",数量："+buyNum+",挂单结果："+sellResultObject, 0, "05cbc8");
    ...
    public List<BigDecimal> getDepth() {
        ...
        String param = "symbol=" + exchange.get("market") + "&step=" + "STEP1" + "&accessKey=" + exchange.get("apikey");//传入参数
        ...
        if (tradesObj != null && tradesObj.getString("status").equals("success")) {
            ...
            List<JSONObject> buyPrices = (List<JSONObject>) jsonData.get("buy");
            List<JSONObject> sellPrices = (List<JSONObject>) jsonData.get("sell");
            ...
            BigDecimal buyMinPri = new BigDecimal(buyPrices.get(fromDepth).getString("price"));
            logger.info("买" + fromDepth + "价格:" + buyMinPri);

            BigDecimal buyMaxPri = new BigDecimal(buyPrices.get(toDepth).getString("price"));
            logger.info("买" + toDepth + "价格:" + buyMaxPri);

            BigDecimal sellMinPri = new BigDecimal(sellPrices.get(fromDepth).getString("price"));
            logger.info("卖" + fromDepth + "价格:" + sellMinPri);

            BigDecimal sellMaxPri = new BigDecimal(sellPrices.get(toDepth).getString("price"));
