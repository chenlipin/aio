...
public class RunTestKline {
    ...
    public void init(int id) {
        //实例化策略对象
        TestKline kline = new TestKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
        ...
        public void dowork() {
            ...
            if (robot != null && redisHelper.getRobot(name).getStatus() == Constant.KEY_ROBOT_STATUS_RUN) {
                ...
                try {
                    ...
                    redisHelper.setParam("Exception_"+ kline.id, redisHelper.getParam("Exception_"+ kline.id) + "|------|"+strs);

//                    if (redisHelper.getParam(kline.id + key) == null) {
//                        redisHelper.setParam(kline.id + key, String.valueOf(System.currentTimeMillis()));
//                    } else if (System.currentTimeMillis() - Long.valueOf(redisHelper.getParam(kline.id + key)) > Constant.KEY_SNS_INTERFACE_ERROR_TIME) {
//                        redisHelper.setParam(kline.id + key + "_true", "true");
//                        commonUtil.sendSms(redisHelper.getRobot(kline.id).getName() + "异常机器人停止");
//                        redisHelper.removeParent(kline.id + key);
//
//                    }
                    //长时间异常，发送短信给我
