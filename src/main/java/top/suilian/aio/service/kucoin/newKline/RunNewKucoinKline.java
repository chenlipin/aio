...
@Component
public class RunNewKucoinKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        NewKucoinKline kline = new NewKucoinKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        NewKucoinKline kline;

        public Work(NewKucoinKline kline) {
            super(kline.id);
            this.kline = kline;
        }
