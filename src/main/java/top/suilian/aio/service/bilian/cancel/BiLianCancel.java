...
public class BiLianCancel extends BiLianParentService {
    public BiLianCancel(
    ...
    public void init() {
        ...
        for (CancelOrder cancelOrder : cancelOrderList) {
            ...
            try {
                ...
                JSONObject orderDetialRes = judgeRes(orderDetial, "code", "selectOrder");
                Integer status = null;
                String remark = orderDetial;
                if (orderDetialRes.getInt("code") == 0) {
                    JSONObject jsondata =orderDetialRes.getJSONObject("data");
                    JSONObject jsonOrderInfo = jsondata.getJSONObject("order_info");
                    if ((jsonOrderInfo.getInt("status"))==4) {  //订单撤销
                        status = Constant.KEY_CANCEL_ORDER_STATUS_CANCELLED;
                    } else if ((jsonOrderInfo.getInt("status"))==2) {  //订单完全成交
                ...
                } else if (orderDetialRes.getInt("code")==1 ){       //订单不存在
