...
public class RunBiLianKline {
    ...
    private RunBiLianKline.Work work;
    private List<RunBiLianKline.Work> works = new ArrayList<RunBiLianKline.Work>();
    ...
    public void init(int id) {
        //实例化策略对象
        BiLianKline kline = new BiLianKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    public boolean stopWork(int id) {
        for (RunBiLianKline.Work work : works) {
            if (work.getName() == id) {
    ...
    class Work extends StopableTask<RunBiLianKline.Work> {
        BiLianKline kline;

        public Work(BiLianKline kline) {
            super(kline.id);
            this.kline = kline;
        }
