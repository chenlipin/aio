...
@Component
public class RunNewCoinvvKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        NewCoinvvKline kline = new NewCoinvvKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        NewCoinvvKline kline;

        public Work(NewCoinvvKline kline) {
            super(kline.id);
            this.kline = kline;
        }
