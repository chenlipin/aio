package top.suilian.aio.service.coinvv.newKline;
...
public class NewCoinvvKline extends CoinvvParentService {
    public NewCoinvvKline(
    ...
    public void openInterval(BigDecimal sellPrice, List<Map<String, Object>> allBids, BigDecimal openIntervalPrice) {

        BigDecimal price;
        for (Map<String, Object> bid : allBids) {

            price = new BigDecimal(String.valueOf(bid.get("price")));
            ...
            } else if (new BigDecimal(exchange.get("openIntervalAllAmount")).compareTo(intervalAmount.add(new BigDecimal(String.valueOf(bid.get("amount"))))) < 0) {
                setRobotArgs(id, "isOpenIntervalSwitch", "0");
                setTradeLog(id, "刷开区间的数量已达到最大值,停止刷开区间", 0, "000000");
                continue;
            }
            ...
            startOpenInterval(new BigDecimal(String.valueOf(bid.get("price"))), new BigDecimal(String.valueOf(bid.get("price"))));
        }
    }
    ...
    private void startOpenInterval(BigDecimal buyPri, BigDecimal buyAmount) {
        ...
        try {
            ...
            JSONObject jsonObject = judgeRes(resultJson, "code", "submitTrade");
            if (resultJson != null && !resultJson.equals("") && jsonObject.getInt("code") == 200) {
                JSONObject data = jsonObject.getJSONObject("data");
                String orderId = data.getString("en_id");
                ...
                if (selectResult != null && selectResult.getInt("code") == 200) {
                    JSONObject data1 = jsonObject.getJSONObject("data");
                    if (data != null) {
                        String status = String.valueOf(data1.getInt("status"));
                        if (status.equals("2")) {
