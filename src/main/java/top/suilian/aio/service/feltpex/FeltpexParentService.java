...
public class FeltpexParentService extends BaseService implements RobotAction {
    public String baseUrl = "https://dapptestapi.bitbridge.pro/";
    ...
    /**
     * {"code":1,"msg":"Success","timeMillis":1705729837852,"data":{"orderId":695,"orderNo":"EB1705729837815140"}}
     *
     * @param type
     * @param price
     * @param amount
    ...
    public String submitTrade(int type, BigDecimal price, BigDecimal amount) {
        ...
        String uri = "/api/spot/createOrder";
        Map<String, String> params = new TreeMap<>();
        params.put("type", "1");
        params.put("symbol", exchange.get("market"));
        params.put("side", type==1?"1":"2");
        ...
        String signature = HMAC.sha256_HMAC(order, exchange.get("tpass"));
        HashMap<String, String> head = new HashMap<>();
        head.put("X-Signature", signature);
        head.put("X-Address",exchange.get("apikey") );
        try {
     ...
     *     "code":1,
     *     "msg":"Success",
     *     "timeMillis":1705730850343,
     *     "data":{
     *         "id":695,
     *         "orderNo":"EB1705729837815140",
     *         "userId":3853,
     *         "entrustType":1,
     *         "symbol":"FET/USDT",
     *         "type":1,
     ...
     *         "cancelTime":0,
     *         "hangStatus":1,
     *         "createdAt":1705729837000,
     *         "updatedAt":1705729837000,
     *         "pairId":4,
     *         "fee":0
     *     }
     * }
    ...
    public String selectOrder(String orderId) {
        String toime = System.currentTimeMillis() + "";
        String uri = "/api/spot/orderDetail";
        Map<String, String> params = new TreeMap<>();
        params.put("orderId",orderId);
        params.put("timestamp",toime );
        String order = splicing(params);
        logger.info("挂单参数" + order);
        String signature = HMAC.sha256_HMAC(order, exchange.get("tpass"));
        ...
        String trade = null;
        try {
            trade = HttpUtil.getAddHead(baseUrl + uri+"?orderId="+orderId+"&timestamp="+toime,  head);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getDepth() {
        return httpUtil.get("https://dapptestapi.bitbridge.pro/api/spot/depth?pairId=4");
    }
     ...
     * 撤单
     * <p>
     * {"code":1,"msg":"Success","timeMillis":1705731253116,"data":null}
     */
    public String cancelTrade(String orderId) {
        String time = System.currentTimeMillis() + "";
        // 输出字符串
        String trade = null;
        String uri = "/api/spot/cancelOrder";
        Map<String, String> params = new TreeMap<>();
        ...
        try {
            trade = HttpUtil.post(baseUrl + uri, params, head);
            setTradeLog(id, "撤单：" +orderId+ ":]=>" + trade, 0, "ff6224");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        logger.info("撤单结束：" + trade);
        return trade;
    ...
    public void setBalanceRedis() {
        ...
        if (balance == null || overdue) {
            ...
            if (obj != null && obj.getInt("error_code") == 0) {
                JSONObject dataJson = obj.getJSONObject("data");
                JSONObject free = dataJson.getJSONObject("free");
                JSONObject freeze = dataJson.getJSONObject("freeze");
                firstBalance = free.getString(coinArr.get(0));
                firstBalance1 = freeze.getString(coinArr.get(0));

                lastBalance = free.getString(coinArr.get(1));
                lastBalance1 = freeze.getString(coinArr.get(1));
