package top.suilian.aio.service.playcoin.newKline;
...
import top.suilian.aio.service.playcoin.PlayCoinParentService;
...
public class NewPlayCoinKline extends PlayCoinParentService {
    public NewPlayCoinKline(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_PLAYCOIN_KLINE, id);
    }
    ...
    public void init() throws UnsupportedEncodingException {
        ...
        if (runTime < timeSlot) {
            ...
            String trades = httpUtil.getAddHead(baseUrl + "/depth.do?symbol=" + exchange.get("market") + "&depth=0&type=0",params);
            ...
            try {
                ...
                if (jsonObject != null && "0".equals(jsonObject.getString("code"))) {
                    ...
                    if (jsonObject1 != null && "0".equals(jsonObject.getString("code"))) {
                        String tradeId1 = jsonObject1.getJSONObject("data").getString("orderId");
                        orderIdTwo = tradeId1;
                        removeSmsRedis(Constant.KEY_SMS_INSUFFICIENT);

                    } else {
                        String res = cancelTrade(tradeId,type+"");
    ...
    public void selectOrderDetail(String orderId, int type) {
        try {
            ...
            if (jsonObject != null&&jsonObject.getInt("code")==0) {
                JSONObject jsonObject1 = jsonObject.getJSONObject("data").getJSONArray("data").getJSONObject(0);
                String status = jsonObject1.getString("orderStatus");
                String fromSource = jsonObject1.getString("fromSource");
                if (status.equals("2")) {
                    setTradeLog(id, "订单id：" + orderId + "完全成交了", 0, "000000");
                    logger.info("订单id：" + orderId + "完全成交了");
                    ...
                    String res = cancelTrade(orderId,fromSource);
                    JSONObject cancelRes = judgeRes(res, "code", "cancelTrade");
    ...
    private void startOpenInterval(BigDecimal buyPri, BigDecimal buyAmount) {
        try {
            ...
            if (jsonObject != null && "true".equals(jsonObject.getString("result"))) {
                String orderId = jsonObject.getString("client_oid");
                ...
                if (selectResult != null&&selectResult.getInt("code")==0) {
                    JSONObject jsonObject1 = selectResult.getJSONObject("data").getJSONArray("data").getJSONObject(0);
                    String status = jsonObject1.getString("orderStatus");
                    String fromSource = jsonObject1.getString("fromSource");
                    if (status.equals("2")) {
                        setTradeLog(id, "订单id：" + orderId + "完全成交了", 0, "000000");
