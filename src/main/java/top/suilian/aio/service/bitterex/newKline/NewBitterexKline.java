package top.suilian.aio.service.bitterex.newKline;
...
public class NewBitterexKline extends BitterexParentService {
    public NewBitterexKline(
...
//                runBitterRandomDepth.init(id + 1);
//            }
        ...
        if (runTime < timeSlot) {
            ...
            String trades = httpUtil.get(baseUrl +"/markets/"+exchange.get("market")+"/orderbook?depth=1");
            ...
            if (tradesObj != null) {
                ...
                BigDecimal buyPri = new BigDecimal(JSONObject.fromObject(com.alibaba.fastjson.JSONObject.toJSONString(bid.get(0))).getString("rate"));
                BigDecimal sellPri = new BigDecimal(JSONObject.fromObject(com.alibaba.fastjson.JSONObject.toJSONString(ask.get(0))).getString("rate"));
            ...
            if (!"0".equals(sellOrderId)) {
                selectOrderDetail(sellOrderId, 0,true);
                sellOrderId = "0";
                if (maxEatOrder != 0) {
                    eatOrder++;
            ...
            if (!"0".equals(buyOrderId)) {
                selectOrderDetail(buyOrderId, 0,true);
                buyOrderId = "0";
                if (maxEatOrder != 0) {
                    eatOrder++;
                    setTradeLog(id, "已吃堵盘口单总数:" + eatOrder + ";吃单成交上限数:" + maxEatOrder, 0);
            ...
            flag = RandomUtils.nextBoolean();
            try {
                String resultJson = submitTrade(type, price, num,flag);
                JSONObject jsonObject = judgeRes(resultJson, "code", "submitTrade");

                if ("OPEN".equals(jsonObject.getString("status"))) {
                    String tradeId = jsonObject.getString("id");
                    ...
                    if (jsonObject1.getString("status")==null||"CLOSED".equals(jsonObject1.getString("status"))) {
                        ...
                        String res = cancelTrade(tradeId,flag);
...
//                        setTradeLog(id, "当前随机值（" + value + ":涨幅）", 1);
    ...
    public BigDecimal getRandomPrice() throws UnsupportedEncodingException {
        ...
        if (tradesObj != null ) {
            ...
            net.sf.json.JSONArray bid = tradesObj.getJSONArray("bid");
            net.sf.json.JSONArray ask = tradesObj.getJSONArray("ask");
            JSONObject buyjsonObject = JSONObject.fromObject(com.alibaba.fastjson.JSONObject.toJSONString(bid.get(0)));
            JSONObject sellObject = JSONObject.fromObject(com.alibaba.fastjson.JSONObject.toJSONString(ask.get(0)));
            BigDecimal buyPri = new BigDecimal(buyjsonObject.getString("rate"));
            BigDecimal sellPri = new BigDecimal(sellObject.getString("rate"));
            ...
            if ("1".equals(exchange.get("isTradeCheck"))) {
                ...
                //吃堵盘口的订单
                BigDecimal buyAmount = new BigDecimal(buyjsonObject.getString("quantity"));
                BigDecimal sellAmount = new BigDecimal(sellObject.getString("quantity"));
                ...
                if (buyAmount.compareTo(new BigDecimal(exchange.get("buyMinLimitAmount"))) < 1 && "1".equals(exchange.get("isBuyMinLimitAmount"))) {
                    if ("0".equals(exchange.get("isSuspendTrade"))) {
                        if (maxEatOrder == 0 || maxEatOrder > eatOrder) {
                            ...
                            try {
                                String sellOrder = submitTrade(2, buyPri, buyAmount,true);
                ...
                if (sellAmount.compareTo(new BigDecimal(exchange.get("sellMinLimitAmount"))) < 1 && "1".equals(exchange.get("isSellMinLimitAmount"))) {
                    ...
                    if ("0".equals(exchange.get("isSuspendTrade"))) {
                        if (maxEatOrder == 0 || maxEatOrder > eatOrder) {
                            ...
                            try {
                                String buyOrder = submitTrade(1, sellPri, sellAmount,true);
...
//    }
    ...
    public void selectOrderDetail(String orderId, int type,Boolean flag) {
        try {
            String str = selectOrder(orderId,flag);
            ...
            if (jsonObject.getString("status")!=null) {
                ...
                if ("CLOSED".equals(status)) {
                    ...
                    String res = cancelTrade(orderId,flag);
                    JSONObject cancelRes = judgeRes(res, "code", "cancelTrade");
