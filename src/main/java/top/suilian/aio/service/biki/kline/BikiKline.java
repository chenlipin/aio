package top.suilian.aio.service.biki.kline;
...
import top.suilian.aio.service.biki.BikiParentService;
...
public class BikiKline extends BikiParentService {
    public BikiKline(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_BIKI_KLINE, id);
    }
    ...
    public void init() {
        ...
        if (runTime < Integer.parseInt(timeSlot)) {
            ...
            //获取深度 判断平台撮合是否成功
            logger.info(market);
            String trades = httpUtil.get(baseUrl + "/open/api/market_dept?symbol=" + market + "&type=step0");
            ...
            if (tradesObj != null && tradesObj.getInt("code") == 0) {
                ...
                JSONObject tick = jsonObject.getJSONObject("data");
                JSONObject tick1 = tick.getJSONObject("tick");
                JSONArray asks = tick1.getJSONArray("asks");
                JSONArray bids = tick1.getJSONArray("bids");


                List<List<String>> buyPrices = (List<List<String>>) asks;

                List<List<String>> sellPrices = (List<List<String>>) bids;
    ...
    public BigDecimal getRandomPrice() {
        ...
        if (tradesObj != null && tradesObj.getInt("code") == 0) {
            ...
            List<List<String>> buyPrices = (List<List<String>>) bids;

            List<List<String>> sellPrices = (List<List<String>>) asks;
            ...
            if (interval.compareTo(minInterval) < 0) {
                ...
                if (price.compareTo(sellPri) < 0 && price.compareTo(buyPri) > 0) {
                    ...
                    judgeSendMessage(isMobileSwitch, message, mobile, Constant.KEY_SMS_SMALL_INTERVAL);
    ...
    public void setParam() {
        ...
        this.minTradeLimit = robotArg(id, "minTradeLimit");
    ...
    public void setPrecision() {
        ...
        if (rtObj != null && rtObj.getInt("code") == 0) {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                String symbol = data1.getString("symbol");
                if (symbol.equals(market)) {

                    precision.put("amountPrecision", data1.getString("amount_precision"));
                    precision.put("pricePrecision", data1.getString("price_precision"));
                }
            }
        }
    }
