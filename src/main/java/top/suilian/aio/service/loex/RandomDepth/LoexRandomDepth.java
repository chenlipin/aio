...
public class LoexRandomDepth extends LoexParentService {

    public LoexRandomDepth(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_LOEX_DEPTH, id);
    }
    ...
    public void init() {
        ...
        if (Integer.parseInt(exchange.get("depthCancelNum")) >= depthCancelOrderNum) {
            ...
            try {
                ...
                if("0".equals(buyResultObject.getString("code"))){
    ...
    public List<BigDecimal> getDepth() {
        ...
        String trades = httpUtil.get("https://open.loex.io/open/api/market_dept?symbol=" + exchange.get("market") + "&type=step0");

        JSONObject tradesObj = judgeRes(trades, "bids", "getRandomPrice");

        if (trades != null && "0".equals(tradesObj.getString("code"))) {
