package top.suilian.aio.service.digifinex.newKline;
...
@Component
public class RunNewDigfinexKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        DigfinexNewKline kline = new DigfinexNewKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        DigfinexNewKline kline;

        public Work(DigfinexNewKline kline) {
            super(kline.id);
            this.kline = kline;
        }
