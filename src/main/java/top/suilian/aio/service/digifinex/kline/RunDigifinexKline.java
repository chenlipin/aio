...
@Component
public class RunDigifinexKline {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        DigifinexKline kline = new DigifinexKline(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        DigifinexKline kline;

        public Work(DigifinexKline kline) {
            super(kline.id);
            this.kline = kline;
        }
