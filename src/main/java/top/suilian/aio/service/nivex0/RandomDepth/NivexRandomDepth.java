package top.suilian.aio.service.nivex0.RandomDepth;
...
public class NivexRandomDepth extends NivexParentService {

    public NivexRandomDepth(
        ...
        super.id = id;
        super.logger = getLogger(Constant.KEY_LOG_PATH_Nivex_DEPTH, id);
    }
...
//        }
        if (Integer.parseInt(exchange.get("depthCancelNum")) >= depthCancelOrderNum) {
            ...
            try {
                ...
                if(resultBuy!=null&&buyResultObject.getInt("status")==200){
                    setTradeLog(id, "深度 买单价格："+depthPrice.get(0)+",数量："+buyNum+",挂单结果："+buyResultObject.getString("msg"), 0, "05cbc8");
            ...
            try {
                ...
                if(resultSell!=null&&sellResultObject.getInt("status")==200){
    ...
    public void JudegOrder(JSONObject object) throws UnsupportedEncodingException {
        if (object != null && object.getInt("status")==200) {
            ...
            if (jsonObject != null && jsonObject.getInt("status") == 200) {
                ...
                if (status == 2) {
                    ...
                    setTradeLog(id, "深度订单撤单[" + tradeId + "]=>" + cancelRes.getString("msg"), 0, "000000");
                }
            }
        }
    }
    ...
    public List<BigDecimal> getDepthlist() {
        ...
        if (tradesObj != null && tradesObj.getInt("status") == 200) {


            JSONObject data = tradesObj.getJSONObject("data");

            JSONObject tick = data.getJSONObject("depth");
            List<List<String>> buyPrices = (List<List<String>>) tick.get("b");

            List<List<String>> sellPrices = (List<List<String>>) tick.get("a");
