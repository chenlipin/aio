...
@Component
public class RunIdcmRandomDepth {
    //region    Service
    @Autowired
    ...
    public void init(int id) {
        //实例化策略对象
        IdcmRandomDepth randomDepth = new IdcmRandomDepth(cancelExceptionService, cancelOrderService, exceptionMessageService, robotArgsService, robotLogService, robotService, tradeLogService, httpUtil, redisHelper, id);
    ...
    class Work extends StopableTask<Work> {
        IdcmRandomDepth randomDepth;

        public Work(IdcmRandomDepth randomDepth) {
            ...
            if (robot != null && redisHelper.getRobot(name).getStatus() == Constant.KEY_ROBOT_STATUS_RUN) {
                ...
                try {
                    ...
                    if (redisHelper.getParam(randomDepth.id + key) == null) {
                        ...
                        String name = redisHelper.getRobot(randomDepth.id).getName();
